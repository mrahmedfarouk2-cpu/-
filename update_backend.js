...
const serverFile = 'c:\\Users\\User\\Downloads\\visual-arts-assessment-portal\\server.js';
let content = fs.readFileSync(serverFile, 'utf8');

const newData = `const defaultData = {
...
const oldDefaultRegex = /const defaultData = {[\s\S]*?};\n\n\/\/ Seeding function/m;
content = content.replace(oldDefaultRegex, newData + '\n\n// Seeding function');

const oldSeedRegex = /async function seedDefaultConfig\(\) {[\s\S]*?}\n\nseedDefaultConfig/m;
const newSeed = `async function seedDefaultConfig() {
  ...
  if (!configRow) {
    ...
    // Force update with new full data to apply changes
    await prisma.systemConfig.update({
      where: { id: configRow.id },
      data: { config: JSON.stringify(defaultData) }
    });
    console.log('System configuration updated to include all visual arts assessment models.');
  }
}

seedDefaultConfig`;

content = content.replace(oldSeedRegex, newSeed);
